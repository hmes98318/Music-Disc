...
export const slashExecute = async (bot: Bot, client: Client, interaction: ChatInputCommandInteraction) => {
    ...
    const SUCCESS = await player.skip();
    return SUCCESS
        ? interaction.editReply({ embeds: [embeds.textMsg(bot, client.i18n.t('commands:MESSAGE_SKIP_SUCCESS'))], allowedMentions: { repliedUser: false } })
        : interaction.editReply({ embeds: [embeds.textMsg(bot, client.i18n.t('commands:MESSAGE_SKIP_FAIL'))], allowedMentions: { repliedUser: false } });
};