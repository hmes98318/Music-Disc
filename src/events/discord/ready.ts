...
// Discord.js version import
// Reading package.json directly from the node_modules path
const pkgPath = path.resolve('./node_modules/discord.js/package.json');
const djsVersion = JSON.parse(fs.readFileSync(pkgPath, 'utf-8')).version;

export default async (bot: Bot, client: Client) => {
    // Discord.js major version check.
    const majorVersion = parseInt(djsVersion.split('.')[0]);
    const readyEvent = majorVersion >= 15 ? 'clientReady' : 'ready';
