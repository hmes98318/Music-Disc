...
export default async (bot: Bot, _client: Client, player: Player, tracks: Track | Track[]) => {
    if (player.playing) {
        if (Array.isArray(tracks)) { // PLAYLIST_LOADED
            ...
            await (player.metadata?.channel as any /* discord.js type error ? (v14.16.2) */).send({ embeds: [embeds.addPlaylist(bot.config.bot.embedsColor, playlist[0].title, subtitle, playlist[0].uri, playlist[0].thumbnail!)] });
        }
        else {
            const track = tracks as Track;
            const subtitle = `Author : **${track?.author}**\nDuration **${track?.duration.label}**\n`;

            await (player.metadata?.channel as any /* discord.js type error ? (v14.16.2) */).send({ embeds: [embeds.addTrack(bot.config.bot.embedsColor, track.title, subtitle, track.uri, track.thumbnail!)] });
        }
